...
基于原有项目fork [yechaoa/BrightnessAndVolume](https://github.com/yechaoa/BrightnessAndVolume)出来的。
目的是给老爷子，一键设置音量到最大，同时模式设置为正常模式。 以下是原有项目readme。

# BrightnessAndVolume

修改系统亮度和音量

| <img src="/gif/brightness.gif" width="330"/> | <img src="/gif/volume.gif" width="330"/> | | :--: | :--: | | 亮度 | 音量 |
...
很简单，设置`window`的属性即可。 这个只会对当前页面有效，返回页面或退到后台，屏幕亮度都会`恢复`到初始值状态。
...
前面讲到的其实是单页面的亮度设置，也可以修改系统的屏幕亮度，即影响所有的页面和app，一般不会有这种操作。 这也涉及到一个`高级`隐私权限，是否`允许修改系统设置`，且需要在app设置页面`手动授权`。
